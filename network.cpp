...
bool network_t::to_net(int src_port, pri_t pri, net_cmd_t cmd) {
  ...
  bool space_p = from_net_fifos[pri][cmd.dest]->space_p();  // true if there is space in the fifo
  if (space_p) from_net_fifos[pri][cmd.dest]->tenqueue(cur_cycle + NETWORK_LATENCY, cmd); // if there is space, enqueue the command

  return(space_p); // if there is space, return true, else return false
